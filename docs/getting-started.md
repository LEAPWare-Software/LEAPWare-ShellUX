...
| **npm** | 10 or newer; 11.16.0 is what the lockfile was written with. **Corrected 2026-09-18: use 11.16.0.** npm 10.9.8 was measured crashing on this lockfile (`npm update` and `npm audit fix`: `Cannot read properties of null (reading 'edgesOut')`); see `docs/traps.md` | Pinned as `packageManager` so a laptop reaching for yarn or pnpm errors instead of silently resolving a different tree from the version ranges in `package.json`. |
