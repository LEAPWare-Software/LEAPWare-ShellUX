...
- [x] **Ruleset as code, not classic branch protection.** `.github/rulesets/main.json` matches sessionkeeper's shape: `~DEFAULT_BRANCH` with `bypass_actors: []`; `deletion` and `non_fast_forward`; `pull_request` with `required_approving_review_count: 0` and squash-only; `required_status_checks`, strict, naming `Verify (ubuntu-latest)`, `Verify (macos-latest)`, `Verify (windows-latest)`, `Browser tests (chromium)` and `Declared Node floor (22.13.0)`; `merge_queue` with ALLGREEN, and `on: merge_group:` added to `ci.yml` and `browser.yml`. `scripts/apply-rulesets.mjs`, a Node port of `lws_apply_rulesets.py` that shells out to `gh api` and supports `--dry-run`, has a `node:test` file. `docs/maintainers/repository-settings.md` covers what each rule enforces, how to re-apply, and the squash-only repo `PATCH`. [C-38]
...
- [ ] Rewrite `SECURITY.md` the way sessionkeeper does: a "Report a vulnerability" link first, then `leapware@outlook.com` (D-03), then scope notes (what the host does and does not claim, citing tests per Amendment G), then supported versions ("1.x: latest release only"). Today's file leads with "Supported versions", and its own "Reporting a vulnerability" section still says the repository is private with `security_and_analysis` `null` — stale since D-34/D-43 made it public.
