...
- [x] Triage the Dependabot PRs. **Done 2026-09-18:** #117 and the group #131 merged green on every leg; the D-28 pin landed in `.github/dependabot.yml` (#130); declined with reasons and an ignore rule: #118, #119, #120, #132, #133, #134, #135, #136, #137, #138, #139. Zero open pull requests after:
...
## Step 7 — Production hardening  (0/5)  — `sonnet`
- [ ] #86 observability, minimum version: `onerror`/`unhandledrejection` in the renderer and `process.on('uncaughtException')` in main, logged to a rotating file under `app.getPath('logs')`. Sourcemaps kept out of the asar and attached to the release. **No telemetry leaves the machine** (no endpoint exists or is declared).
- [ ] #85 for Electron: set Vite `build.target` to the Chromium version Electron 43 ships. The Safari half is moot because the target is Electron only. Verify that `base` is correct for `file://` loading by launching the packaged app.
