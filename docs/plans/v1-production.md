...
## Step 3 — Go public, on the `leapware-sessionkeeper` pattern  (12/16)
...
- [ ] Write the D-27 reversal honestly in `docs/maintainers/repository-settings.md` or `docs/DECISIONS.md`: bootstrap is documented as owner-only there, but the D-27 "no branch protection, and no spend to get it" decision that this ruleset reverses is not named anywhere in the settings doc.
...
- [x] `hasBotMergeComment` (scripts/claims/pr-evidence.mjs, D-53) considers only the single latest `claude[bot]` comment, sorted by `created_at`, and reads the LAST occurrence of `Reviewed SHA:` and `Verdict:` within it. This is **entry-point validation**, not an integrity control: it is real at the `PR evidence` check (a missing review, a stale SHA, or a review not authored by `claude[bot]` is rejected there), and it says nothing about any other route a comment claiming that login could arrive by. [C-44]
