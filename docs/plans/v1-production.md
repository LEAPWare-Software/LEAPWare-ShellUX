...
- [x] **Clear the red production audit** (found 2026-09-18: `audit-schedule.yml`'s six scheduled Monday runs from 2026-08-10 through 2026-09-14 all failed on `js-yaml`), close the unaudited dev tree, and make a failing scheduled audit file an issue. PR #126: `verify` exit 0, review MERGE, Medium finding fixed; merged `e00ea93`, all seven checks green. [C-04]
- [x] Triage the Dependabot PRs. Every Dependabot pull request opened through 2026-09-18 — 30 in total, #34 to #140 — is merged or closed, none left open; the D-28 pin landed separately in `.github/dependabot.yml` as the human-authored #130 (MERGED). It does not mean the queue stays empty going forward: Dependabot can open a new PR at any time after this date. [C-05]
...
- [x] React 19 (#106) and `eslint-plugin-react-refresh` 0.5 (#105) stay deferred to after 1.0: the lockfile resolves React and React DOM to 18.3.1, not a stray 19.x from a transitive bump. [C-06]
...
- [x] Rollout step 2: every throwaway-PR case shown with its run id, each matching its designed pass/fail outcome. Cases (a) to (l): `docs/claims-evidence/rollout-step2-2026-09-19.md` lists every run and the specific reason read from its log by hand; the row itself re-reads only each run's conclusion from GitHub, not the specific reason, which the API does not expose to an allowlisted check. [C-30]
...
- [x] PR B: both checks required with `integration_id: 15368`, applied and read back. Merged as #179 and applied with `scripts/apply-rulesets.mjs`; ruleset 23685990 now requires seven contexts, including "Prove claims" and "PR evidence", each pinned to the GitHub Actions app. Drift against `.github/rulesets/main.json` is `compare-ruleset.mjs`'s ongoing job, run as the `S-ruleset` synthetic row on every completed main run (docs/proof-of-completion.md §3.5) — not re-run by this row, which is `github`-class and cannot invoke that `node` check under the argv allowlist. [C-37]

## Step 1 — Record the decisions before acting on them  (2/2)
- [x] In `docs/DECISIONS.md`, add D-31 through D-34 (one per row above, "Called by: Owner, 2026-09-18"). Strike through the superseded D-09, D-11 (as the v1 gate), D-12 and D-27, and rewrite §3 as the new line of sight: this plan's steps. Add D-35: **contract gaps ship as documented limits** (#16, #17, #28, #32, #57, #80, #91). This is consistent with D-23 (no third parties). #65 is re-scoped to the 1.1 milestone. [C-44]
- [x] Create GitHub milestone **v1.0.0** and move every issue this plan closes onto it; what is deferred goes to **1.1**. The milestone is the gate, live-checked: 0 open issues currently carry no milestone. [C-25]

## Step 2 — Context files within their caps  (4/5)
- [x] HANDOFF.md → ≤3000 bytes. It keeps only the transition: where main is, what landed, what is in flight, the next step. §2–§12 move verbatim (relative-link rewrites aside) to `docs/history/handoff-archive-2026-08.md`. [C-08]
- [x] CLAUDE.md → ≤200 lines. The "Traps" body moves to `docs/traps.md`, and CLAUDE.md keeps a pointer to it plus the rules. [C-09]
- [ ] The "jsdom is blind" body also moves to `docs/traps.md`, with CLAUDE.md keeping only a pointer. Not done: as of this audit (2026-09-19) that body is still in full in CLAUDE.md.
...
- [x] Citations must still resolve: moved text keeps its `*Tests:*` markers. [C-11]

## Step 3 — Go public, on the `leapware-sessionkeeper` pattern  (16/16)
...
- [x] Scan the published history for secrets with gitleaks: every commit on `main`, plus the lockfile as it stands, which `gitleaks git` cannot read because `.gitattributes` marks it `-diff`. This is a one-off invocation, not a tracked dependency (ADR-0002). Any hit is **rotated**; deleting it from history is not enough. `.gitleaks.toml` allowlists 3 confirmed-false `generic-api-key` matches on the `"key": "<name>"` shape in `docs/claims.json`, scoped to that exact shape and path. Re-run 2026-09-19 with the allowlist: no leaks. Raw output, the commit counts, and the triage of the 3 false positives are in the row's evidence file. [C-19]
...
- [x] **Ruleset as code, not classic branch protection.** `.github/rulesets/main.json` targets the default branch (`conditions.ref_name.include: ["~DEFAULT_BRANCH"]`) with `bypass_actors: []`; `deletion` and `non_fast_forward`; `pull_request` with `required_approving_review_count: 0` and squash-only; `required_status_checks`, strict, naming `Verify (ubuntu-latest)`, `Verify (macos-latest)`, `Verify (windows-latest)`, `Browser tests (chromium)` and `Declared Node floor (22.13.0)`; `merge_queue` with ALLGREEN, and `on: merge_group:` added to `ci.yml` and `browser.yml`. `scripts/apply-rulesets.mjs`, a Node port of `lws_apply_rulesets.py` that actually shells out to `gh api` (not a stub) and supports `--dry-run`, and has a `node:test` file. `docs/maintainers/repository-settings.md` covers what each rule enforces, how to re-apply, and the squash-only repo `PATCH`. [C-38]
- [x] Write the D-27 reversal honestly in `docs/maintainers/repository-settings.md` or `docs/DECISIONS.md`: bootstrap is documented as owner-only there, but the D-27 "no branch protection, and no spend to get it" decision that this ruleset reverses is not named anywhere in the settings doc. [C-45]
- [x] Private vulnerability reporting is enabled on the repository, measured with `gh api repos/LEAPWare-Software/LEAPWare-ShellUX/private-vulnerability-reporting`. [C-39]
- [x] Secret scanning and Dependabot security updates are enabled on the repository, read with an admin token on 2026-09-19 (`security_and_analysis`). CI's read-only token gets `null` for these fields, so this row is dated, not re-proven, and CI would not notice either being turned off. [C-43]
- [x] `hasBotMergeComment`'s review-shaped-comment selection: `PR evidence` fails unless the LATEST *review-shaped* `claude[bot]` comment (a markup-tolerant `Claude review` opener, OR a line-anchored `Reviewed SHA:` line, OR a line-anchored `Verdict:` line) carries `Reviewed SHA:` equal to the head SHA and a clean `Verdict: MERGE`; an unrelated, non-review-shaped later `claude[bot]` reply is skipped rather than treated as "the latest" (D-55 round 3, fixing F7). This is **entry-point validation**, not an integrity control: real at the `PR evidence` check, silent about every other route a comment claiming that GitHub login could arrive by. [C-46]
- [x] Rewrite `SECURITY.md` the way sessionkeeper does: a "Report a vulnerability" link first, then `leapware@outlook.com` (D-03), then scope notes (what the host does and does not claim, citing tests per Amendment G), then supported versions ("1.x: latest release only"). Done 2026-09-25: "Reporting a vulnerability" is now the first section (link, then the D-03 email, then the unchanged reporting content); the trust-model, "defends against" and "does NOT defend against" sections follow, unchanged; "Supported versions" is now last before "Where the evidence lives", and no longer overclaims a release that has not happened — it still states plainly that there has been no release and no tag (checked against `package.json`'s `0.1.0` and an empty `git tag -l`), and adds the future policy in the plan's own words: once a release exists, only the latest `1.x` line gets fixes, the equivalent of "1.x: latest release only". [C-47]
- [x] #74 ("Branch protection is impossible on this repository...") and #103 ("Decide the update feed...") are closed. [C-40]
- [x] #74 and #103 are marked done with evidence (API responses pasted). Split from the README item on 2026-09-19: both closing comments state the read-back values in prose (visibility `PUBLIC`, ruleset 23685990 and its rules), and neither pastes a raw API response. Done 2026-09-25: pasted the literal `gh api repos/LEAPWare-Software/LEAPWare-ShellUX --jq '{visibility,private}'` output and the literal `gh api repos/LEAPWare-Software/LEAPWare-ShellUX/rulesets/23685990` (and `/rulesets`) output into `docs/claims-evidence/C-49-repo-visibility-and-ruleset-2026-09-25.txt`; the ruleset id named in this line still resolves directly, no discrepancy. [C-49]
...
- [x] README top matter in sessionkeeper's order: a one-paragraph what-it-is, a license line, an Installing link (`docs/INSTALL.md`, Step 7), a Developing block (`npm ci && npm run verify`), and Contributing/Security links. The long body moves to `docs/`. Done 2026-09-25: the "Status" table, the "What it is" diagram, "Getting started" and "Developing" table moved verbatim to `docs/readme-body.md`, and README now runs opening paragraph → license → Installing link → the two-line `npm ci && npm run verify` block → Documentation/Contributing/Security, with a new Documentation-table row pointing at `docs/readme-body.md`. [C-48]
...
- [x] The CTO confirms or overrides the SHAPE-BRIEF gate points: §3 editorial voice, §4 the 11–13px scale, §9 palette arrow-keys, §13.1 (the owner delegated these calls to the CTO on 2026-09-19). Each goes into DECISIONS.md. [C-22]
...
- [x] Stream A: repairs #112 (title overprint) and #113 (title outside the token system, dark contrast), and addresses #111 (contrast gate measured against a background the app never paints), which stays open. Landed in the charts PR (#148), which also touched shared files (`design/tokens/semantic.json`, `design/check-contrast.mjs`, `package.json`) — not disjoint from other streams. #146 (Database chart clipped) filed, not fixed. [C-15]
...
## Step 6b — Runtime plugin host  (2/7)  — `opus` (design discovery, security-relevant)
...
- [x] Integrity: the manifest carries a `sha512` of the bundle, and the loader refuses a mismatch. This is **entry-point validation** at install, not an integrity control against a hostile local user. The claim is written only once a named test pins it. [C-42]
...
- [x] #85 for Electron: set Vite `build.target` to the Chromium version Electron 43 ships. The Safari half is moot because the target is Electron only. Verify that `base` is correct for `file://` loading by launching the packaged app. `chrome150`, measured from Electron 43.2.0; the packaged renderer loads over the `shellux://` scheme, not `file://`. The packaged launch is measured, not just recorded: `docs/measurements/csp-2026-09-18.json` (via `scripts/csp-smoke.mjs`) shows zero CSP violations on both surfaces, real DOM content extracted from the Mail and Database fixtures, a drawn chart canvas, a shown tooltip, a completed pointer drag and an opened command palette — the renderer loaded and ran, not merely the executable starting. [C-26]
...
- [x] `docs/INSTALL.md` for operators: install per-user, the SmartScreen "More info → Run anyway" path for an unsigned build, where the logs live, and how updates arrive (partly covers #43). Written with those sections plus uninstalling, and linked from the README; the update section honestly says updates don't work yet, whatever the current reason (originally no feed configured; now a configured feed with no release ever published, per plan step 8 items 2-5, PR #233). [C-36]

## Step 8 — Release engineering  (4/6)  — `sonnet`
...
- [x] `publish: { provider: github, owner, repo }` in `electron-builder.yml`. Update exactly the three places `docs/RELEASE.md` §1 names (`DOCUMENTED_ENDPOINTS` in `scripts/check-portability.mjs`, and its fixture in `scripts/__tests__/check-portability.test.mjs`). Both gates must go red, then green. [C-50]
- [x] A separate `release.yml` triggered by `v*` tags (as sessionkeeper does): run `verify` before releasing, then package. `desktop.yml` stays the non-publishing PR build. On a `v*` tag, build Windows only (D-26) and create a **draft** GitHub Release. Upload the installer and `.blockmap` first and `latest.yml` last (RELEASE.md §2.4). Publishing the draft stays a human step. The macOS leg is removed from release publishing (kept or dropped as a CI build, stated either way). [C-51]
- [x] Record in `SECURITY.md` and `docs/RELEASE.md` that unsigned updates depend on `sha512` + HTTPS + GitHub account control, and that this is a **guardrail**. Name the test that pins it, or narrow the claim (Amendment G). [C-52]
- [x] Rewrite RELEASE.md §0 and §1 to the new truth in the same commit (rule 3). [C-53]
