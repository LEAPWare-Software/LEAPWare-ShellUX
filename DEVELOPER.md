...
| `label` | `string` | **Untrusted display text.** Non-blank, at most 256 characters. **New in host contract 2.0 (D-56, #172):** a `label` carrying a bidi override, a C0/C1 control, a line/paragraph separator or nothing but invisible characters is `INVALID_FIELD`, same rule and same `validateText` door as blueprint `name`. *Test:* `src/core/__tests__/validation.test.ts` — "rejects a bidi override in a NavigationNode.label the same way it rejects one in name (D-56, #172)". |
